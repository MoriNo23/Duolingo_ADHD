...
[![Version](https://img.shields.io/badge/version-2.8.0-orange)](https://greasyfork.org/es-419/scripts/590127)
...
| **Segments per lesson** | **5** (4 separators) | How many pieces the bar is split into. 1–12. Fewer = longer chunks; more = more frequent feedback. |
...
| **Show ⚡ FAST** | **on** | Shows a "⚡ RÁPIDO / FAST" tag when a segment is closed on Diamond or Super. |
...
| **Tramos por lección** | **5** (4 separadores) | En cuántos trozos se divide la barra. 1–12. Menos = trozos más largos; más = feedback más seguido. |
...
| **Mostrar ⚡ RÁPIDO** | **activado** | Muestra la etiqueta "⚡ RÁPIDO" cuando un tramo se cierra en Diamante o Super. |
...
This repo publishes only the production userscript (`duolingo-adhd.user.js`) plus this README and the demo media. Development lives next to it but is git-ignored and never pushed:

* `duolingo-adhd-dev.user.js` — source of truth (edit here, never prod directly)
* `duolingo-adhd.test.js` — `node --test duolingo-adhd.test.js` (89 tests, must stay green)
* `mutation-test.js` — mutation coverage check
* `build.py` — `python3 build.py` regenerates prod from dev
* `docs/` — `DESIGN.md`, `UI-BRIEF.md`, `UI-PROPOSAL.md`
* `openspec/` + `vault/` — planning history and project journal (local only)

> Do not run `git clean -fdx`: ignored dev/docs files are not backed up by git.
